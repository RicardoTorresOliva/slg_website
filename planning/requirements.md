...
| RF-33 | Cada formulario público incluye un campo trampa (honeypot) invisible para personas; un envío con ese campo relleno **o con el campo ausente** se descarta silenciosamente y no crea `lead_capture`. La ausencia delata al script que postea contra la API sin leer el formulario; ambas responden igual. *(Ampliado por D-164, 2026-09-21.)* | D-16, D-164 | explícito | pública |
...
| RF-149 | El inicio del portal («Hoy») muestra, para la empresa del usuario: noticias del día por importancia con su comentario, próximo hito de cada proyecto activo, pendientes abiertos, últimos entregables y avisos; cada bloque con estado vacío redactado. | Ricardo 2026-09-18, spec-delta Academy | explícito | portal |
| RF-150 | Una noticia pertenece a **una** empresa; lleva título, fuente, resumen, comentario para esa empresa, importancia 1–3 y fecha; la escribe un agente con `news:write` o un usuario de SLG desde HQ; nunca se lista fuera de su empresa. | spec-delta Academy | explícito | portal, API |
| RF-151 | Cada proyecto puede tener hitos (título, fecha, `pending\|done`, orden) y pendientes (título, fecha límite, `open\|done`, `closes_by` `client\|slg`); el portal los muestra en «Programa»; el cliente marca `done` **solo** los pendientes con `closes_by = client`. | spec-delta Academy | explícito | portal |
| RF-152 | HQ permite crear y editar hitos, pendientes y noticias de una empresa sin necesitar un agente. | spec-delta Academy, DoD #9 | explícito | HQ |
| RF-153 | La API v1 expone `POST /organizations/{id}/news` (`news:write`), `POST /projects/{id}/milestones`, `PATCH /milestones/{id}`, `POST /projects/{id}/action-items`, `PATCH /action-items/{id}` (`milestones:write`), con las reglas de DU-22/23 y descritos en `openapi.json`. | spec-delta Academy, B.5 | explícito | API |
| RF-154 | HQ tiene «Conexiones»: los productos externos de SLG (nombre, descripción, URL) leídos de un archivo del repo; añadir uno es editar un archivo. | spec-delta Academy, D-11 | explícito | HQ |
| RF-155 | El portal tiene «Clases»: materiales de programa agrupados por proyecto, los de vídeo distinguidos y abiertos en pestaña nueva; pasa por `materialesPorProyecto()`. | spec-delta Academy, DU-20 c.2 | explícito | portal |
| RF-156 | Toda escritura de noticia, hito o pendiente —por API o por HQ— queda en `audit_log` con su actor. | spec-delta Academy, RNF-29 | explícito | transversal |
