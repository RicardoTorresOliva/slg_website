...
| R-41 | **La aplicación usa las credenciales de administrador de MinIO (`MINIO_ROOT_USER`/`PASSWORD`), no una clave con alcance mínimo.** El Console gratuito de MinIO retiró la gestión de claves de acceso de su interfaz web (cambio de producto, 2026); crear una clave acotada exige la CLI `mc`, que no está disponible sin acceso por terminal al VPS o al contenedor. **Resuelto por D-51**: Ricardo elige la credencial de administrador como solución temporal, por ahora — mismo tipo de aplazamiento que S-01/R-08, con la misma condición: revisar antes del go-live. | Media | Alto | Las credenciales viven **solo** en la variable de entorno de Easypanel, nunca en el repo (mismo freno de secretos que las demás) · **antes del go-live**, crear una clave acotada con `mc admin accesskey create` (necesita acceso por terminal al VPS o exec al contenedor de `minio`) y rotar `S3_ACCESS_KEY_ID`/`S3_SECRET_ACCESS_KEY` (`.env.example`) sin tocar `MINIO_ROOT_PASSWORD` · mientras tanto, un fallo en la capa de la aplicación (no solo un secreto filtrado) podría dar acceso de administrador completo a MinIO, más allá de los dos buckets `downloads`/`deliverables`. | Se acerca el go-live con la aplicación aún usando la credencial de administrador · alguien necesita rotar `MINIO_ROOT_PASSWORD` y descubre que rompe también la aplicación, prueba de que no están separadas. | Ricardo |
