...
cover: null
social:
  hook: "Approving an AI initiative without asking for the baseline is signing blind."
  linkedin: "A director does not need to know how to implement artificial intelligence. They need to know what to ask whoever proposes it, and to recognise the disqualifying answer. This article brings the questions, the pilot signals and the minimum measurement dashboard."
  x: "A director does not implement AI. They ask. And they recognise the disqualifying answer."
...
1. **Request the inventory of AI systems in operation, with one name per system.** Not a department: a person. An agent without an owner is shadow AI with a budget [source: Organización Humano-Agente La Empresa Híbrida y el Rediseño del Trabajo].
2. **Request the five indicators from section 5 for each one.** If they do not exist, that is the board's first decision, and it is cheaper than any new project.
3. **Apply the questionnaire in section 4 to the proposal on the table today**, not to the next one. Record in the minutes which questions went unanswered.
4. **Request the breakdown of the AI portfolio** between licenses and models on one side, and redesign, training and oversight on the other. The ratio is the diagnosis.
5. **Demand the measured baseline of a single process before the end of the quarter.** One process, chosen for its volume and its cost, not for its visibility.
6. **Put on record the basis of every AI-assisted recommendation the board receives.** Traceability of the algorithmic recommendation is today a documented fiduciary obligation, not good practice [source: AI Literacy A Multi-Dimensional Analysis of Governance, Revenue Systems, and Epistemic Rigor in the Agentic Era].
7. **Execute one act of destruction, with date and signature.** A process that ends, a contract that is cancelled, a report that is no longer produced. The difference between an organization that says it destroys and one that destroys is always that signature [source: DAL OS v1 0].
