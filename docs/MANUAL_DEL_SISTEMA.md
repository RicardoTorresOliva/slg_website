...
| Correo | `MAIL_SMTP_HOST`, `MAIL_SMTP_PORT`, `MAIL_SMTP_USERNAME`, `MAIL_SMTP_PASSWORD`, `MAIL_FROM_ADDRESS`, `MAIL_FROM_NAME`, `MAIL_REPLY_TO`, `MAIL_ALERTS_TO`, `MAIL_LEADS_TO` | Transporte SMTP (Resend) · remitente en `mailweb` · dirección de respuesta · avisos operativos · buzón del cliente para las capturas de un sitio sin CRM (paso 5b) |
