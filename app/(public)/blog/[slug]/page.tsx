...
export function generateStaticParams() {
  return listarArticulos("es").map((a) => ({ slug: a.slug }));
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const a = buscarArticulo(slug, "es");
  return a ? { title: a.titulo, description: a.descripcion } : {};
}

export default async function PaginaDeArticulo({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  return <Articulo slug={slug} locale="es" />;
}
