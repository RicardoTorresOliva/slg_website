...
/** Términos de uso (DU-06). Pública y sin autenticación: las pantallas de consentimiento OAuth la exigen (RF-12). */
export async function generateMetadata(): Promise<Metadata> {
  const r = loadCollection<{ title: string; description: string; pair: string | null }>("page", "es")
    .find((p) => p.slug === "terminos");
  return r ? { title: r.data.title, description: r.data.description } : {};
}

export default function PaginaDeTerminos() {
  return <PaginaDeTexto slugEs="terminos" locale="es" />;
}
