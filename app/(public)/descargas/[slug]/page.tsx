...
export function generateStaticParams() {
  return listarDocumentos("es").map((d) => ({ slug: d.slug }));
}
...
export async function generateMetadata({
  ...
  const doc = buscarDocumento(slug, "es");
  return doc ? { title: doc.titulo || undefined, description: doc.publico || undefined } : {};
}

export default async function PaginaDeDocumento({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  return <FichaDeDocumento slug={slug} locale="es" />;
}
