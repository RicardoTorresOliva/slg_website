...
| ~~4~~ | ✅ **Resuelto por FU-11** (2026-09-11): tabla `rate_limit_event` (`drizzle/0009_fu11_anti_abuso.sql`) — una fila por intento (`action`, `key_kind`, `key_value`, `created_at`), ventana deslizante contada en el momento de comprobar, sin lectura-modificación-escritura. RLS en `0010_anti_abuso_rls.sql`: solo `system`. Ver `lib/anti-abuse/rate-limit.ts` | FU-11 |
| ~~5~~ | ✅ **Resuelto por FU-11** (2026-09-11): tabla `blocked_email_domain` (`domain` como clave primaria), sembrada con la misma lista PROVISIONAL que ya vivía en `components/download-form/free-email-domains.ts`. Editable sin desplegar (un `INSERT`, verificado en `scripts/db/test-anti-abuse.ts`). RLS: solo `system` hasta que exista una pantalla de HQ para editarla | FU-11 |
